...
<main>
  <% if(loggedAdminOn){%>
  <form name="newBando" action="Dispatcher" method="post" style="margin-bottom: 0;">
    ...
    <%for(int i=0; i<bandoList.size(); i++){%>
      <%if((loggedAdminOn) || (LocalDate.now().isBefore(LocalDate.parse(bandoList.get(i).getDataScadenzaIscrizione().toStringSQL()) ))){ %>
        ...
        <li style="display: flex; align-items: center;">
          <form name="bandoView<%=bandoList.get(i).getId()%>" action="Dispatcher" method="post" style="margin-bottom: 0; width: 100%; display: flex; align-items: center; justify-content: center;">
            <!--
        ...
        <%// TODO: permettere all'amministratore di visualizzare bandi scaduti%>

        <li style="display: flex; align-items: center; margin-top: 10px">
          <% if(loggedAdminOn){%>
          <form name="bandoDelete<%=bandoList.get(i).getId()%>" action="Dispatcher" method="post" style="margin-bottom: 0; margin-left: 1rem; padding: 0; display: flex;">
          ...
          <form name="bandoModify<%=bandoList.get(i).getId()%>" action="Dispatcher" method="post" style="margin-bottom: 0; margin-left: 1rem; padding: 0; display: flex;">
